...
export default function Turnero() {
  ...
  const [servicios, setServicios] = useState([]); // Ahora dinámico
  const [configHorarios, setConfigHorarios] = useState([]); // Horarios de la DB
  ...
  async function fetchInitialData() {
    ...
    // Traer Configuración de Horarios
  ...
  // Generar slots de tiempo basados en la apertura y cierre de la DB
  const generateSlots = () => {
  ...
  const handleReserve = async () => {
    ...
    if (!error) {
      ...
      setForm({ name: form.name, phone: form.phone }); // Mantenemos datos por comodidad
      getAppointments();
    }
  };
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <header className="flex flex-col md:flex-row items-center justify-between mb-8 px-6">
          <div className="text-center md:text-left">
            ...
            <p className="text-[11px] font-bold text-slate-400 uppercase tracking-[0.3em] mt-1 text-center md:text-left">
        ...
        <div className="bg-white rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col md:flex-row min-h-[750px]">
          ...
          {/* COLUMNA CENTRAL: CALENDARIO Y HORARIOS */}
        ...
        <footer className="mt-12 pb-8 border-t border-slate-200 pt-8 flex flex-col md:flex-row justify-between items-center gap-6 px-6">
          <div className="text-center md:text-left">
            <h4 className="text-xl font-black text-slate-900 lowercase italic">ejemplo<span className="text-indigo-600">.barber</span></h4>
            <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mt-1">Estilo y Precisión en cada corte</p>
          </div>
          <div className="text-center md:text-right">
            <p className="text-[10px] text-slate-400 font-medium tracking-tight">© 2026 — Villa Constitución, Santa Fe.</p>
