        ...
        <div className="bg-white rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col md:flex-row min-h-[750px]">
          <div className="w-full md:w-1/4 p-8 flex flex-col bg-slate-50/50 border-r border-slate-100">
            ...
            <div className="space-y-3">
              ...
              <button onClick={handleReserve} disabled={!form.name || !form.phone || !selectedSlot} className={`w-full font-black py-4 rounded-2xl shadow-xl transition-all mt-4 ${(!form.name || !form.phone || !selectedSlot) ? 'bg-slate-200 text-slate-400' : 'bg-slate-900 text-white active:scale-95 hover:bg-black'}`}>RESERVAR AHORA</button>
            </div>
          </div>
          ...
          <div className="w-full md:w-1/4 flex flex-col border-l border-slate-100 bg-white">
            <div className="p-6">
              <a href="https://wa.me/543400000000" target="_blank" rel="noreferrer" className="flex items-center justify-center gap-3 w-full bg-green-500 text-white font-black uppercase text-[10px] py-5 rounded-2xl shadow-lg hover:bg-green-600 transition-all"><MessageCircle className="w-5 h-5" /> WhatsApp Barbería</a>
            ...
            <div className="flex-1 bg-slate-50 p-6 rounded-t-[3rem] border-t border-slate-100 mt-4">
              <h3 className="font-black text-[10px] uppercase tracking-widest text-slate-900 mb-4 flex items-center gap-2"><Search className="w-3 h-3 text-indigo-600" /> Mis Reservas</h3>
